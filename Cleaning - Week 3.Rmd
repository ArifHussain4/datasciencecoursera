...
- Use library(reshape2) 
    - melt data set --> pass df to a melt function, tell it which are id variables and which are measure
    - creates a bunch of id values
...
```
#Casting data frames
- dcast --> recast the data set into a particular shape/df


```{r}
cylData <- dcast(carMelt, cyl ~ variable) #cyl row, variable columns
cylData #summarizes data by count

cylData2 <- dcast(carMelt, cyl ~ variable, mean)
cylData2 #summarizes data by mean
...
tapply(InsectSprays$count, InsectSprays$spray, sum) #apply count along index spray, sum
      #within each value of spray it sums the count

spIns <- split(InsectSprays$count, InsectSprays$spray) #this splits by spray (group by)
    #will give a list
spIns

sprCount <- lapply(spIns, sum) #apply a function to the list values; same as above
sprCount #gives a list

unlist(sprCount) #unlist the list; brings back to vector

sapply(spIns, sum) #does the lapply and unlist in one step
```

#plyr package good interface to do above in one step

```{r}
ddply(InsectSprays,.(spray), summarize, sum=sum(count))
      #pass dataset through ddply function
      #.(variables we want summarized)
      #summarize the variable, how? sum=sum(count) --> sum up the count within the variable

spraySums <- ddply(InsectSprays, .(spray), summarize, sum=ave(count,FUN=sum))
    #ave function --> gives same dimension as dataset, can now add to dataset and run analyses
dim(spraySums) 

head(spraySums)
```

##Dplyr package --> dataframes
- arrange() ---> reorder rows
- filter() ---> extract subset of rows based on logical conditions
- select() ---> return subset of columns of df
- mutate() ---> add new variables/columns or transform existing variables
- rename() ---> rename variable
- summarize() ---> summary stats

- Optimized version of plyr package; very fast

- don't need to use $ after referencing the column name
- result is a new df
...
names(cars_dplyr)
head(select(cars_dplyr, mpg:drat))
head(select(cars_dplyr, -(mpg:drat)))

#regular R:
i <- match("mpg", names(cars_dplyr))
j <- match("drat", names(cars_dplyr))

head(cars_dplyr[,-(i:j)])


#Filter
car.dpfilter <- filter(cars_dplyr, mpg > 21)
car.dpfilter

car.dpfilter2 <- filter(cars_dplyr, mpg > 21, cyl == 6)
car.dpfilter2


#arrange (asc/desc)
car.dparrange <- arrange(cars_dplyr, mpg)
head(car.dparrange)
tail(car.dparrange)

car.dparrange2 <- arrange(cars_dplyr, desc(mpg))
head(car.dparrange2)
tail(car.dparrange2)


#rename
car.rename <- rename(cars_dplyr, display = disp)
head(car.rename)


#mutate
cars.mutate <- mutate(cars_dplyr, mpg_hp = mpg*hp/100) #create a new variable
head(cars.mutate)


#groupby --> split DF according to parameters
cars.groupby <- group_by(cars_dplyr, cyl)
cars.groupby
summarize(cars.groupby, mean_mpg = mean(mpg, na.rm=TRUE), max_hp = max(hp, na.rm = TRUE))

#use mutate to create year variable
## --> mutate(xyz, year=as.POSIClt(date)$year + 1900)
## then can group by etc..

##month variable --> x %>% mutate(month = as.POSIXlt(date)$month + 1) %>% summarize(...)
...
- use merge()
    ...
    - all=TRUE --> if value in one but not in other, it should be included with NA (outer join)
    
- default to merge on all column names
    - insersect()
        - intersect(names(solutions), names(reviews))
        
- can use join
    - only works with common names 
    - arrange(join((df1, df1),id))
    
- multiple dataframes
    - dfList = list(df1, df2, df3)
    - join_all(dfList) --> if we have common id
...
##merging id from solutions to solution_id to reviews
    ##
